...
const getIdAndTitle = (newReleases) => {
  let newArray = [];
  newReleases.forEach(movie => {
    newArray.push({ id: movie.id, title: movie.title });
  });
  return newArray;
};
...
// use the map method.
const getIdAndTitleWithMap = (newReleases) => {

  return newReleases.map(movie => { return { id: movie.id, title: movie.title } });
};
console.log(getIdAndTitleWithMap(newReleases));
